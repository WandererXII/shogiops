...
import { IllegalSetup } from '../src/position/position.js';
...
test('rules - parse sfen and back', () => {
  RULES.forEach((rules) => {
    const pos = parseSfen(rules, initialSfen(rules), false).unwrap();
    expect(makeSfen(pos)).toEqual(initialSfen(rules));
  });
});
...
test('partial sfen', () => {
  ...
  expect(remadePartialSfen).toEqual(partialSfen);
  expect(pos.hands).toEqual(Hands.empty());
  expect(pos.turn).toEqual('sente');
  expect(pos.moveNumber).toEqual(1);
  expect(parseSfen('standard', 'lnsgkgsnl/9/9/9/9/9/9/9/LNSGKGSNL b - ', true).isOk).toEqual(true);
});
...
test('invalid sfen', () => {
  ...
  expect(pos.isErr).toEqual(true);
  expect(pos.isErr ? pos.error.message : undefined).toEqual(IllegalSetup.InvalidPiecesDoublePawns);
});

test('parse and make sfen', () => {
  [
...
test('chushogi sfen', () => {
  ...
  expect(parseSfen('chushogi', '12/12/7k3p/12/7K4/12/12/9+E2/12/3X8/12/12 b', true).isOk).toEqual(
    true,
  );
  expect(parseSfen('chushogi', '12/12/7k3p/12/7K4/12/12/9K2/12/3X3+E4/12/12 b', true).isOk).toEqual(
    false,
  );
});
