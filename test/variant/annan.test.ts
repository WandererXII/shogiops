...
test('capture attacker move giver', () => {
  ...
  const posS = parseSfen('annan', '5k3/9/9/5l3/5p3/9/2B6/9/5K3 b - 1').unwrap();
...
test('capture attacker move giver - with king', () => {
  ...
  const pos2 = parseSfen('annan', '9/8k/9/9/9/9/9/p+r7/K8 b - 1').unwrap();
...
test('do not allow discovering check by capturing move giver', () => {
  const pos = parseSfen('annan', '3k5/9/9/3p5/3bG4/9/9/6K2/9 b').unwrap();
  expect(pos.moveDests(parseSquareName('5e')).has(parseSquareName('6d'))).toBe(false);
  expect(perft(pos, 1, false)).toBe(13);

  const pos2 = parseSfen('annan', '6k2/p8/9/3Bs4/3R5/9/9/6K2/9 w').unwrap();
