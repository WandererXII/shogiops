  ...
  if (isDrop(move)) {
    ...
    if (piece) {
      const roleStr = roleToWestern(pos.rules)(piece.role),
        disambStr = piecesAiming(pos, piece, move.to).without(move.from).isEmpty() ? '' : makeNumberSquare(move.from),
      ...
      if (defined(move.midStep)) {
        ...
        if (igui) return `${roleStr}${disambStr}!${makeNumberSquare(move.midStep)}`;
