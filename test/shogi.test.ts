...
test('impasse', () => {
  const pos = parseSfen('standard', '2SGS4/+B+RGKGLLRB/3G5/9/7pp/8k/9/9/9 b - 1').unwrap();
  expect(pos.isImpasse()).toBe(true);

  // Only 27 points
  const pos2 = parseSfen('standard', 'G3+R3S/GG5SS/GLPBKBPLS/9/9/7+p+p/7+pk/7+p+p/9 b - 1').unwrap();
  expect(pos2.isImpasse()).toBe(false);

  // Only 26 points
  const pos3 = parseSfen('standard', '9/9/9/9/9/9/3r1lllg/+P+P1+bkssgg/K+P4ssg w r 2').unwrap();
  expect(pos3.isImpasse()).toBe(false);

  // Not 10+ pieces in prom zone
  const pos4 = parseSfen('standard', '2SGS4/+B+RGKG2RB/9/9/7pp/8k/9/9/9 b g2s4n4l16p 1').unwrap();
  expect(pos4.isImpasse()).toBe(false);

  // King not in prom zone
  const pos5 = parseSfen('standard', '2SGS4/+B+RG1G2RB/3G5/9/7pp/8k/9/9/4K4 b - 1').unwrap();
  expect(pos5.isImpasse()).toBe(false);

  // King not in prom zone
  const pos6 = parseSfen('standard', '2SGS4/+B+RG1G2RB/3G5/9/7pp/8k/9/9/4K4 b - 1').unwrap();
  expect(pos6.isImpasse()).toBe(false);

  // Enough with hand
  const pos7 = parseSfen('standard', 'G8/4K4/PPPPPPPPP/9/9/7ss/7sk/9/9 b 2R2B 1').unwrap();
  expect(pos7.isImpasse()).toBe(true);

  // 27 points gote
  const pos8 = parseSfen('standard', '9/9/9/9/9/9/3r1llll/+P+P1+bkssgg/K+P3ssgg w r 2').unwrap();
  expect(pos8.isImpasse()).toBe(true);

  // 28 points sente
  const pos9 = parseSfen('standard', 'G3+R3S/GG2P2SS/GLPBKBPLS/9/9/7+p+p/7+pk/7+p+p/9 b - 1').unwrap();
