...
test('fromRank', () => {
  ...
  expect(SquareSet.fromRank(1)).toEqual(
    new SquareSet([0xffff0000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(2)).toEqual(new SquareSet([0x0, 0xffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]));
  expect(SquareSet.fromRank(3)).toEqual(
    new SquareSet([0x0, 0xffff0000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(4)).toEqual(new SquareSet([0x0, 0x0, 0xffff, 0x0, 0x0, 0x0, 0x0, 0x0]));
  expect(SquareSet.fromRank(5)).toEqual(
    new SquareSet([0x0, 0x0, 0xffff0000, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(6)).toEqual(new SquareSet([0x0, 0x0, 0x0, 0xffff, 0x0, 0x0, 0x0, 0x0]));
  expect(SquareSet.fromRank(7)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0xffff0000, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(8)).toEqual(new SquareSet([0x0, 0x0, 0x0, 0x0, 0xffff, 0x0, 0x0, 0x0]));
  expect(SquareSet.fromRank(9)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0xffff0000, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(10)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0xffff, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(11)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000, 0x0, 0x0])
  );
  expect(SquareSet.fromRank(12)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff, 0x0])
  );
  expect(SquareSet.fromRank(13)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000, 0x0])
  );
  expect(SquareSet.fromRank(14)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff])
  );
  expect(SquareSet.fromRank(15)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000])
  );
});
...
test('fromFile', () => {
  ...
  expect(SquareSet.fromFile(8)).toEqual(
    new SquareSet([
      0x1000100, 0x1000100, 0x1000100, 0x1000100, 0x1000100, 0x1000100, 0x1000100, 0x1000100,
    ])
  );
  expect(SquareSet.fromFile(9)).toEqual(
    new SquareSet([
      0x2000200, 0x2000200, 0x2000200, 0x2000200, 0x2000200, 0x2000200, 0x2000200, 0x2000200,
    ])
  );
  expect(SquareSet.fromFile(10)).toEqual(
    new SquareSet([
      0x4000400, 0x4000400, 0x4000400, 0x4000400, 0x4000400, 0x4000400, 0x4000400, 0x4000400,
    ])
  );
  ...
  expect(SquareSet.fromFile(12)).toEqual(
    new SquareSet([
      0x10001000, 0x10001000, 0x10001000, 0x10001000, 0x10001000, 0x10001000, 0x10001000,
      0x10001000,
    ])
  );
  expect(SquareSet.fromFile(13)).toEqual(
    new SquareSet([
      0x20002000, 0x20002000, 0x20002000, 0x20002000, 0x20002000, 0x20002000, 0x20002000,
      0x20002000,
    ])
  );
...
test('ranksAbove', () => {
  ...
  expect(SquareSet.ranksAbove(1)).toEqual(
    new SquareSet([0xffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.ranksAbove(2)).toEqual(
    new SquareSet([0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.ranksAbove(3)).toEqual(
    new SquareSet([0xffffffff, 0xffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.ranksAbove(4)).toEqual(
    new SquareSet([0xffffffff, 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.ranksAbove(5)).toEqual(
    new SquareSet([0xffffffff, 0xffffffff, 0xffff, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  expect(SquareSet.ranksAbove(6)).toEqual(
    new SquareSet([0xffffffff, 0xffffffff, 0xffffffff, 0x0, 0x0, 0x0, 0x0, 0x0])
  );
  ...
  expect(SquareSet.ranksAbove(13)).toEqual(
    new SquareSet([
      0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffff, 0x0,
    ])
  );
...
test('ranksBelow', () => {
  ...
  expect(SquareSet.ranksBelow(2)).toEqual(
    new SquareSet([
      0x0, 0xffff0000, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
    ])
  );
  ...
  expect(SquareSet.ranksBelow(9)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0xffffffff, 0xffffffff, 0xffffffff])
  );
  expect(SquareSet.ranksBelow(10)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000, 0xffffffff, 0xffffffff])
  );
  expect(SquareSet.ranksBelow(11)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffffffff, 0xffffffff])
  );
  expect(SquareSet.ranksBelow(12)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000, 0xffffffff])
  );
  expect(SquareSet.ranksBelow(13)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffffffff])
  );
  expect(SquareSet.ranksBelow(14)).toEqual(
    new SquareSet([0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xffff0000])
  );
...
test('shl256', () => {
  ...
  expect(
    new SquareSet([0x1, 0x8000, 0x10000, 0x80000000, 0x0, 0x0, 0x0, 0x80000000]).shl256(1)
  ).toEqual(new SquareSet([0x2, 0x10000, 0x20000, 0x0, 0x1, 0x0, 0x0, 0x0]));
  ...
  expect(bigS.shl256(1)).toEqual(
    new SquareSet([
      0x8400780, 0x10201020, 0x8001000, 0x2000400, 0x800100, 0x200040, 0x10200020, 0x7800840,
    ])
  );
...
test('rbit256', () => {
  ...
  const random1 = [
    0xc8866bcd, 0x1ec29f93, 0xf5ddebe3, 0x8f0ada65, 0x373d2c52, 0xa6a16ef5, 0x8d1f9954, 0x4ab3e8c7,
  ];
  ...
  const random2 = [
    0x83b5bee6, 0x19bf9b6c, 0x0e6c109c, 0x6d21e29b, 0x95cc034c, 0x5b8e8497, 0xe2758b39, 0xfa201e44,
  ];
