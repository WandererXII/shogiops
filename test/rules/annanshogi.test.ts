...
import { fullSquareSet } from '../../src/position/util.js';
...
test('annanshogi perft', () => {
  annanPerfts.forEach(([sfen, depth, res]) => {
...
test('do not allow capturing move givers that prevent check', () => {
  ...
  expect(pos.moveDests(parseSquareName('4d')).has(parseSquareName('5c'))).toEqual(false);
});
...
test('drop', () => {
  ...
  expect(pos.dropDests({ color: 'gote', role: 'pawn' }).has(parseSquareName('4c'))).toEqual(true);
  expect(pos.isLegal({ role: 'pawn', to: parseSquareName('4c') })).toEqual(true);

  const pos2 = parseSfen('annanshogi', '4k4/9/4G4/9/9/9/9/9/9 b P 1').unwrap();
  expect(pos2.dropDests({ color: 'sente', role: 'pawn' }).has(parseSquareName('5b'))).toEqual(
    false,
  );
  expect(pos2.isLegal({ role: 'pawn', to: parseSquareName('5b') })).toEqual(false);
  ...
  expect(pos3.isLegal({ role: 'pawn', to: parseSquareName('5a') })).toEqual(true);
  expect(pos3.isLegal({ role: 'lance', to: parseSquareName('5a') })).toEqual(true);
  expect(pos3.isLegal({ role: 'knight', to: parseSquareName('5a') })).toEqual(true);
  expect(pos3.isLegal({ role: 'knight', to: parseSquareName('5b') })).toEqual(true);

  const pos4 = parseSfen('annanshogi', '9/9/9/9/9/k8/n1PPPPPP1/N2G2B2/K8 b 3P 1').unwrap();
  expect(pos4.dropDests({ color: 'sente', role: 'pawn' }).has(parseSquareName('8e'))).toEqual(true);
  expect(pos4.dropDests({ color: 'sente', role: 'pawn' }).has(parseSquareName('5e'))).toEqual(
    false,
  );
  expect(pos4.dropDests({ color: 'sente', role: 'pawn' }).size()).toEqual(23);
  expect(pos4.isLegal({ role: 'pawn', to: parseSquareName('5a') })).toEqual(false);
  expect(pos4.isLegal({ role: 'pawn', to: parseSquareName('1a') })).toEqual(true);
  expect(pos4.moveDests(parseSquareName('6g')).size()).toEqual(3);
  expect(pos4.isLegal(parseUsi('6g5f')!)).toEqual(true);
});

test('promotions', () => {
  const pos = parseSfen('annanshogi', '9/3PL4/2P5N/k6N1/g8/G8/K8/9/5L3 b - 1').unwrap();
  expect(pos.isLegal(parseUsi('6b6a')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('6b6a+')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('5b5a')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('5b5a+')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('2d3b')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('2d3b+')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('1c2a')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('1c2a+')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('4i4a')!)).toEqual(true);
  expect(pos.isLegal(parseUsi('4i4a+')!)).toEqual(true);
});

test('capture attacker move giver', () => {
  const pos = parseSfen('annanshogi', '5k3/9/9/7b1/5P3/5L3/9/9/5K3 w - 1').unwrap();
  expect(pos.moveDests(parseSquareName('2d')).has(parseSquareName('4f'))).toEqual(true);
  const posS = parseSfen('annanshogi', '5k3/9/9/5l3/5p3/9/2B6/9/5K3 b - 1').unwrap();
  expect(posS.isLegal(parseUsi('7g4d')!)).toEqual(true);
  expect(perft(posS, 1)).toEqual(5);
  // 2 checkers
  const pos2 = parseSfen(
    'annanshogi',
    'P1G+N3s1/lp7/2+N+Pp+P1+L+P/1sB1kP3/N1pl1s3/g6p1/+p1B2+p+p+l+p/1+s+r+n4R/1g1pK1+p2 b G3P 167',
  ).unwrap();
  expect(perft(pos2, 1)).toEqual(2);

  expect(pos2.isLegal(parseUsi('1h6h')!)).toEqual(true);
  expect(pos2.isLegal(parseUsi('7g6h')!)).toEqual(true);

  const pos3 = parseSfen('annanshogi', '8k/9/3l5/9/9/9/9/2G+n2+p2/3pK4 b - 1').unwrap();
  expect(perft(pos3, 1)).toEqual(1);

  const pos4 = parseSfen('annanshogi', '8k/9/3ll4/9/9/9/4+R4/2G+p2+p2/3pK4 b').unwrap();
  expect(perft(pos4, 1)).toEqual(1);

  const pos5 = parseSfen('annanshogi', '9/9/2B3B2/9/4k4/9/2B3B2/9/8K w').unwrap();
  expect(pos5.isLegal(parseUsi('5e5d')!)).toEqual(true);
  expect(pos5.isLegal(parseUsi('5e5f')!)).toEqual(true);
  expect(pos5.isLegal(parseUsi('5e4e')!)).toEqual(true);
  expect(pos5.isLegal(parseUsi('5e6e')!)).toEqual(true);
});

test('capture attacker move giver - with king', () => {
  const pos = parseSfen('annanshogi', '9/7Pk/7+R1/9/9/9/9/9/8K w - 1').unwrap();
  expect(pos.moveDests(parseSquareName('1b')).has(parseSquareName('2c'))).toEqual(true);

  const pos2 = parseSfen('annanshogi', '9/8k/9/9/9/9/9/p+r7/K8 b - 1').unwrap();
  expect(pos2.moveDests(parseSquareName('9i')).has(parseSquareName('8h'))).toEqual(true);
});
...
test('do not allow discovering check by capturing move giver', () => {
  ...
  expect(pos.moveDests(parseSquareName('5e')).has(parseSquareName('6d'))).toEqual(false);
  expect(perft(pos, 1)).toEqual(13);

  const pos2 = parseSfen('annanshogi', '6k2/p8/9/3Bs4/3R5/9/9/6K2/9 w').unwrap();
  expect(pos2.moveDests(parseSquareName('5d')).has(parseSquareName('6e'))).toEqual(false);
  expect(perft(pos2, 1)).toEqual(10);
});
...
test('allow moving/capturing square behind attacker with king', () => {
  ...
  expect(pos.moveDests(parseSquareName('5f')).has(parseSquareName('6f'))).toEqual(true);
  expect(perft(pos, 1)).toEqual(7);
  const pos2 = parseSfen('annanshogi', '5K3/9/9/9/3B5/3Bk4/9/9/9 w - 1').unwrap();
  expect(pos2.moveDests(parseSquareName('5f')).has(parseSquareName('6f'))).toEqual(true);
});

test('proper parse', () => {
  const pos1 = parseSfen('annanshogi', '9/4kP3/5+R3/2B6/9/9/9/9/8K w - 1', true);
  expect(pos1.isOk).toEqual(true);

  const pos2 = parseSfen('annanshogi', '3P1PNL1/7N1/5P3/9/4p4/6k2/9/3K5/6nsl b', true);
  expect(pos2.isOk).toEqual(true);
});
