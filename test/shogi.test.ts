...
test('blockers perft', () => {
  const posLance = Shogi.fromSetup(parseSfen('4k4/4g4/9/4L4/9/9/9/4K4/9 w - 1').unwrap()).unwrap();
  const posRook = Shogi.fromSetup(parseSfen('4k4/4g4/9/4R4/9/9/9/4K4/9 w - 1').unwrap()).unwrap();
  expect(perft(posLance, 1, false)).toBe(5);
  expect(perft(posRook, 1, false)).toBe(5);
});

test('capturing', () => {
  const pos = Shogi.fromSetup(parseSfen('4k4/9/3g5/3K5/9/9/9/9/9 b - 1').unwrap()).unwrap();
...
test('promotion', () => {
  ...
  const pos2 = Shogi.fromSetup(
    parseSfen('lnsgkgsn1/1r5b1/pppppp1p1/6p2/8L/9/PPPPPPPP1/1B5R1/LNSGKGSN1 b LPp 9').unwrap()
  ).unwrap();
...
test.each(random)('random perft: %s: %s', (_, sfen, d1, d2) => {
  const pos = Shogi.fromSetup(parseSfen(sfen).unwrap()).unwrap();
  expect(perft(pos, 1, false)).toBe(d1);
  expect(perft(pos, 2, false)).toBe(d2);
});

test('pawn checkmate', () => {
  const pos = Shogi.fromSetup(parseSfen('3rkr3/9/8p/4N4/1B7/9/1SG6/1KS6/9 b LPp 1').unwrap()).unwrap();
  ...
  // Single king
  const skPos = Shogi.fromSetup(parseSfen('3rkr3/9/8p/4N4/1B7/9/1SG6/2S6/9 b LPp 1').unwrap()).unwrap();
...
test('impossible checker alignment', () => {
  ...
  const r1 = Shogi.fromSetup(parseSfen('r8/4s4/7k1/b8/9/2K6/3b5/9/9 b - 1').unwrap());
  expect(
    r1.unwrap(
      _ => undefined,
      err => err.message
  ...
  Shogi.fromSetup(parseSfen('9/9/2s4k1/9/6N2/9/9/3K5/7L1 w - 2').unwrap()).unwrap();
});

test('impasse', () => {
  const pos = Shogi.fromSetup(parseSfen('2SGS4/+B+RGKGLLRB/3G5/9/7pp/8k/9/9/9 b - 1').unwrap()).unwrap();
  expect(pos.isImpasse()).toBe(true);

  // Only 27 points
  const pos2 = Shogi.fromSetup(parseSfen('G3+R3S/GG5SS/GLPBKBPLS/9/9/7+p+p/7+pk/7+p+p/9 b - 1').unwrap()).unwrap();
  expect(pos2.isImpasse()).toBe(false);

  // Only 26 points
  const pos3 = Shogi.fromSetup(parseSfen('9/9/9/9/9/9/3r1lllg/+P+P1+bkssgg/K+P4ssg w r 2').unwrap()).unwrap();
  expect(pos3.isImpasse()).toBe(false);

  // Not 10+ pieces in prom zone
  const pos4 = Shogi.fromSetup(parseSfen('2SGS4/+B+RGKG2RB/9/9/7pp/8k/9/9/9 b g2s4n4l16p 1').unwrap()).unwrap();
  expect(pos4.isImpasse()).toBe(false);

  // King not in prom zone
  const pos5 = Shogi.fromSetup(parseSfen('2SGS4/+B+RG1G2RB/3G5/9/7pp/8k/9/9/4K4 b - 1').unwrap()).unwrap();
  expect(pos5.isImpasse()).toBe(false);

  // King not in prom zone
  const pos6 = Shogi.fromSetup(parseSfen('2SGS4/+B+RG1G2RB/3G5/9/7pp/8k/9/9/4K4 b - 1').unwrap()).unwrap();
  expect(pos6.isImpasse()).toBe(false);

  // Enough with hand
  const pos7 = Shogi.fromSetup(parseSfen('G8/4K4/PPPPPPPPP/9/9/7ss/7sk/9/9 b 2R2B 1').unwrap()).unwrap();
  expect(pos7.isImpasse()).toBe(true);

  // 27 points gote
  const pos8 = Shogi.fromSetup(parseSfen('9/9/9/9/9/9/3r1llll/+P+P1+bkssgg/K+P3ssgg w r 2').unwrap()).unwrap();
  expect(pos8.isImpasse()).toBe(true);

  // 28 points sente
  const pos9 = Shogi.fromSetup(parseSfen('G3+R3S/GG2P2SS/GLPBKBPLS/9/9/7+p+p/7+pk/7+p+p/9 b - 1').unwrap()).unwrap();
