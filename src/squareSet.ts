...
export class SquareSet implements Iterable<Square> {
  ...
  static promotionZones(): SquareSet { // backranks
  ...
  shr81(shift: number): SquareSet {
	...
	if (shift >= 27) return new SquareSet(this.mid >>> (shift - 27) ^ (this.hi << (54 - shift)), this.hi >>> (shift - 27), 0);
	if (shift > 0) return new SquareSet(this.lo >>> shift ^ (this.mid << (27 - shift)),
  ...
  shl81(shift: number): SquareSet {
    ...
    if (shift >= 27) return new SquareSet(0, this.lo << (shift - 27), this.mid << (shift - 27) ^ (this.lo >>> (54 - shift)));
    if (shift > 0) return new SquareSet(this.lo << shift, (this.mid << shift) ^ (this.lo >>> (27 - shift)), (this.hi << shift) ^ (this.mid >>> (27 - shift)));
    return this;
  }
